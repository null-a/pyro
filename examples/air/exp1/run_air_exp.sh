...
do
    ...
    python main.py -n 400 -b 100 -blr 0.01 --z-pres-prior 0.01 --scale-prior-sd 0.2 --predict-net 200 --bl-predict-net 200 --decoder-output-use-sigmoid --decoder-output-bias -2 --eval-every 4 --cuda
    mv history.json elbo_$i.json

    git checkout ../../pyro/distributions/torch_distribution.py

    # using prior of 0.5 here. 0.01 seems to hinder performance,
    # though intermediate values might plausiable yield better
    # performance.

    python main.py -n 105 -b 100 --z-pres-prior 0.5 --scale-prior-sd 0.2 --predict-net 200 --bl-predict-net 200 --decoder-output-use-sigmoid --decoder-output-bias -2 --eval-every 1 --rws --no-baselines --cuda
