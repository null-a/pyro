...
from modules import MLP, SquishStateParams, FixedTransition, TransitionDMM, InputRNN, EncodeRNN, Decoder, Combine, CombineDMM, LinearTransition, Combine5, InitialState, BasicTransition, BasicTransition2, ParamW
...
class DynAIR(nn.Module):
    def __init__(self,
        ...
        self.encode_rnn = EncodeRNN(self.window_size, self.num_chan,
                                    self.encode_rnn_hid_size,
                                    self.w_size, self.y_size, self.z_size)

        # self.decode = nn.Sequential(
        #     MLP(self.y_size, [100, self.num_chan * self.window_size**2], nn.ReLU),
        #     nn.Sigmoid())
        ...
        # TODO: Think about what arch. is required here for this data.
        self.decode_bkg = nn.Sequential(
            MLP(self.w_size, [100, (self.num_chan - 1) * self.image_size**2], nn.ReLU),
            nn.Sigmoid())
        ...
        self.decode_obj = Decoder(self.w_size, self.y_size, self.z_size,
                                  self.decoder_hidden_layers, self.num_chan, self.window_size, -2)
        ...
        self.w_param = ParamW(self.input_rnn_hid_size, [50], self.w_size)
    ...
    def model(self, batch, do_likelihood=True):
        ...
        # Scene background code.
        w = self.model_sample_w(batch_size)
        bkg = self.model_generate_bkg(w)

        z = self.model_sample_z_0(batch_size)
        frame_mean = self.model_emission(w, y, z, bkg)
    ...
    def model_generate_bkg(self, w):
        batch_size = w.size(0)

        bkg_rgb = self.decode_bkg(w).view(batch_size, self.num_chan - 1, self.image_size, self.image_size)
        bkg_alpha = batch_expand(self.bkg_alpha, batch_size)

        #print(bkg_rgb.size())
        #print(bkg_alpha.size())

        bkg = torch.cat((bkg_rgb, bkg_alpha), dim=1)

        #print(bkg.size())
        return bkg

    def model_sample_w(self, batch_size):
        return pyro.sample('w',
                           dist.normal,
                           self.w_prior_mean,
                           self.w_prior_sd,
    ...
    def model_emission(self, w, y, z, bkg):
        assert w.size(0) == y.size(0) == z.size(0) == bkg.size(0)
        obj = self.decode_obj(w, y, z)
        z_where = z[:, 0:self.z_where_size]
        return over(self.window_to_image(z_where, obj), bkg)
